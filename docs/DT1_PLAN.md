...
### 1. Plantilla de respuesta automática (`auto_reply_template`)
...
**Solución:**
- Agregar campo `auto_reply_body` en la configuración del tenant (texto exacto de la plantilla).
- Al evaluar si un mensaje `out` cuenta como primera respuesta humana, excluirlo si:
  1. `messages.is_automated = true`, O
  2. El cuerpo del mensaje coincide exactamente con `tenant.auto_reply_body` (comparación normalizada: trim + lowercase).

**Tabla afectada:** `tenants` → agregar columna `auto_reply_body TEXT NULL`.
...
|---|---|---|
| 1 | `add_auto_reply_body_to_tenants` | Columna `auto_reply_body TEXT NULL` |
